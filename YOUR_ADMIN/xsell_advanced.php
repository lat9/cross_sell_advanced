...
<?php
			...
			while (!$xsell_items->EOF) {
				...
				echo '<td style="border-bottom:1px dashed #cccccc;">' . $xsell_items->fields['products_model'] . '</td>';
			           ...
			           echo '<td style="border-bottom:2px;">' . zen_draw_input_field('products_xsell_sort_order['. $xsell_items->fields['products_id'] . ']', $xsell_items->fields['products_xsell_sort_order']) . '</td>';
                                }
...
//			echo '<a href="' . zen_href_link(FILENAME_XSELL_ADVANCED, 'xID=' . $xsell_items->fields['products_id'] . '&action=edit', 'NONSSL') . '">' . zen_image(DIR_WS_IMAGES . 'icons/admin_edit.gif', 'Edit') . '</a>&nbsp;';
				echo '<a href="' . zen_href_link(FILENAME_XSELL_ADVANCED, zen_get_all_get_params(array('action')) . 'xID=' . $xsell_items->fields['ID'] . '&action=delete_cross', 'NONSSL') . '">' . zen_image(DIR_WS_IMAGES . 'icons/delete.gif', 'Delete Cross-Sell') . '</a>';
				echo '</td>';
				echo '</tr>';
...
<?php
		  ...
		  while (!$xsell_products->EOF) {
			...
			echo '<a href="' . zen_href_link(FILENAME_XSELL_ADVANCED, zen_get_all_get_params(array('cID', 'action')) . 'cID=' . $xsell_products->fields['products_id'] . '&action=delete', 'NONSSL') . '">' . zen_image_button('button_delete.gif', 'Delete Cross Sell') . '</a>';
			echo '</td>';
			echo '</tr>';
